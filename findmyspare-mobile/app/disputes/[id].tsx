...
const STATUS_META: Record<DisputeStatus, { label: string; color: string; bg: string }> = {
  open: { label: "Open", color: C.accentInk, bg: "#FFF1DB" },
  under_review: { label: "Under review", color: C.accentInk, bg: "#FFF1DB" },
  return_approved: { label: "Return approved", color: C.success, bg: "#DCF3E6" },
  return_rejected: { label: "Return rejected", color: C.danger, bg: "#FCE8E9" },
  resolved: { label: "Resolved", color: C.success, bg: "#DCF3E6" },
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} className="flex-1">
        <ScrollView className="flex-1" contentContainerClassName="px-5 pb-12 pt-3" keyboardShouldPersistTaps="handled">
          <Card className="flex-row items-center justify-between">
            <View>
              <Text className="text-[11px] text-ink-3">Issue</Text>
              <Text className="text-[14px] font-medium text-ink">{ISSUE_LABEL[dispute.issueType] ?? dispute.issueType}</Text>
            </View>
          ...
          <Text className="text-[12px] mono uppercase text-ink-3 tracking-[0.08em] mt-6 mb-2">Buyer&apos;s report</Text>
          <Card>
            <Text className="text-[13px] text-ink-2 leading-[19px]">{dispute.description}</Text>
            <Text className="text-[10px] text-ink-3 mt-2">{new Date(dispute.createdAt).toLocaleString()}</Text>
          </Card>

          {dispute.supplierResponse ? (
            <>
              <Text className="text-[12px] mono uppercase text-ink-3 tracking-[0.08em] mt-6 mb-2">Supplier&apos;s response</Text>
              <Card><Text className="text-[13px] text-ink-2 leading-[19px]">{dispute.supplierResponse}</Text></Card>
            </>
          ) : null}

          {canRespond ? (
            <Card className="gap-3 mt-5">
              <Text className="text-[12px] mono uppercase text-ink-3 tracking-[0.08em]">Respond</Text>
          ...
          {!isSupplier && dispute.status === "open" ? (
            <Text className="text-[12px] text-ink-3 text-center mt-5">Waiting for the supplier to respond.</Text>
          ) : null}
        </ScrollView>
