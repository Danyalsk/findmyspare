...
import type { Message, Conversation, MessageAttachment } from "@/lib/types";
...
export async function getThread(
  ...
  page = 1,
  limit = 50
): Promise<{ messages: Message[]; user: { id: string; name: string; role: string; businessName: string | null; image: string | null }; pagination: { page: number; limit: number; total: number; totalPages: number } }> {
