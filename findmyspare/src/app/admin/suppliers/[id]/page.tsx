...
export default function AdminSupplierDetailPage() {
  ...
  async function reject() {
    ...
    setActing(true);
    try {
      await adminApi.reject(id, { reason: reason.trim() });
  ...
  return (
    <div className="flex-1 flex flex-col pb-32">
      ...
      <div className="px-5 max-w-3xl w-full mx-auto space-y-4">
        ...
        {/* Live GSTIN verification (Sandbox) */}
        <GstVerificationPanel supplier={supplier} />
...
function GstVerificationPanel({ supplier }: { supplier: User }) {
  const v = supplier.gstVerification;
  if (!v) {
    return (
      <Card className="!p-4">
        ...
        <div className="text-[13px] text-ink-3">
          Not run (Sandbox API not configured, or verification still processing).
          Format + checksum already passed on submit.
        </div>
      </Card>
    );
  }

  const statusActive = (v.status || "").toLowerCase().includes("active");
  return (
    <Card className="!p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="text-xs mono uppercase tracking-[0.06em] text-ink-3">
          GST verification (live)
        </div>
        {v.ok ? (
          <Chip variant={statusActive ? "ok" : "warn"}>{v.status || "Unknown"}</Chip>
        ) : (
          <Chip variant="danger">Lookup failed</Chip>
        )}
      </div>
      {v.ok ? (
        <>
          <Row label="Registered legal name" value={v.legalName} />
          <Row label="Trade name" value={v.tradeName} />
          <Row label="Registered address" value={v.address} />
          <div className="flex justify-between gap-3 py-2 text-[13px]">
            <span className="text-ink-3">Name match</span>
            <span
            ...
            >
              {v.nameMatch === true ? "✓ matches business name" : v.nameMatch === false ? "✗ mismatch — review" : "—"}
            </span>
          </div>
        ...
        <div className="text-[13px] text-[color:var(--danger)]">{v.error || "Could not verify GSTIN."}</div>
      )}
    </Card>
  );
}
