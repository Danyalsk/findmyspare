...
const statusConfig = {
  ...
  responded: { label: "Quotes in", variant: "ok" as const },
      ...
      {loading ? (
        ...
        <div className="px-5 pt-20 flex flex-col items-center text-center gap-4">
          ...
          <div>
            ...
            <div className="text-[12px] text-ink-3 mt-1">Post your first inquiry and get quotes from suppliers</div>
          </div>
          <Link href="/buyer/requests/new">
                          ...
                          {(inquiry.bidCount || 0) > 0 ? (
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-[12px] font-medium text-accent-ink">
                                {inquiry.bidCount} quote{inquiry.bidCount !== 1 ? "s" : ""} received
                              </span>
                            </div>
                          ) : (
                            <span className="text-[11px] text-ink-3">Waiting for quotes...</span>
                          )}
                  ...
                  <Link key={inquiry.id} href={`/buyer/requests/${inquiry.id}`}>
                    <Card className="!p-4 cursor-pointer hover:border-accent/40 transition-colors opacity-60">
                      ...
                      <div className="text-[11px] text-ink-3 mt-2">
                        {inquiry.bidCount || 0} quotes · {timeAgo(inquiry.createdAt)}
                      </div>
                    </Card>
                  </Link>
                ))}
