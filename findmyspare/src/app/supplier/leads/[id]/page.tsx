...
export default function LeadDetailPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      toast.error(getErrorMessage(err, "Could not submit quote."));
    } finally {
      setLoading(false);
    }
  }
    ...
    return (
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 px-8 text-center">
          ...
          <div className="serif text-[26px] leading-[1.1]">Quote submitted!</div>
          <div className="text-[13px] text-ink-3 leading-[1.5]">
            Buyer will review your quote for <span className="font-medium text-ink">{inquiry.partName}</span>.
          </div>
      ...
      <div className="px-5 pb-8 flex flex-col gap-4">
        <Card className="!p-4">
          ...
          <div className="mt-2 flex items-center gap-2">
            <Chip>{inquiry.bidCount || 0} quotes so far</Chip>
          </div>
        </Card>

        <div className="flex flex-col gap-3">
          <div className="mono text-[10px] text-ink-3 tracking-[0.12em]">YOUR QUOTE</div>
          ...
          <Card variant="accent" className="!p-3">
            <div className="flex gap-2.5 items-start">
              ...
              <div className="text-xs text-accent-ink leading-[1.4]">
                <span className="font-semibold">Submit your best quote.</span>{" "}
                The buyer reviews all quotes and connects with you directly.
              </div>
            </div>
          </Card>

          <Button variant="primary" block loading={loading} onClick={handleSubmit}>
            Submit quote{price ? ` · ${formatPrice(parseFloat(price) || 0)}` : ""}
          </Button>
        </div>
      </div>
    </div>
